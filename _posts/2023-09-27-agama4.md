...
After publishing Agama 3 once a month ago, it is time for a new release. Among other things, this
new version fixes several issues in the startup process, allows the use of a network proxy, adds
...
Till now, that section was basically the same as the storage section for the YaST configuration
file. However, regarding to storage management, Agama use cases are only a subset of all the
possibilities YaST offers. Some YaST settings do not make sense for Agama.

For that reason, Agama now implements its own storage settings, making the product configuration
more straightforward and less error-prone.

Additionally, this change has a direct impact in Agama's auto-installation profiles.
...
you face when using Agama (similar to the ancient `save_y2logs` for YaST). Typing `agama logs store`
creates a tarball you can attach to your bug reports.
...
support. Many of the pieces are already in place but we did not make it for version
4. However, we trust that our next major version will feature a translated web interface. Actually,
   our translators [are already working](https://l10n.opensuse.org/projects/agama/) on that.
